...
from ss_baselines.tools.config_tool import get_config,find_best_ckpt_idx

from run import get_config_dict,default_list
...
def main():
    ...
    parser.add_argument(
        ...
        #required=True,
        default='spl',
        help="metric name for the best ckpt selection",
    )
    ...
    ckpt_msg = ""
    if args.eval_best:
        best_ckpt_idx = find_best_ckpt_idx(os.path.join(args.model_dir, 'tb'), metric_name = args.metric_name)
        best_ckpt_path = os.path.join(args.model_dir, 'data', f'ckpt.{best_ckpt_idx}.pth')
        ckpt_msg = f"best {args.metric_name} ckpt:{best_ckpt_path}"
        print("ckpt_msg:",ckpt_msg)
    ...
    if os.path.exists(config.RUN_CONF_FILE):
        os.remove(config.RUN_CONF_FILE)

    with open(config.RUN_CONF_FILE, 'w') as f:
